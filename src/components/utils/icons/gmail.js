import React from "react";

function IconGmail() {
  return (
    <svg
      id="IconGmail"
      xmlns="http://www.w3.org/2000/svg"
      x="0"
      y="0"
      enableBackground="new 0 0 512 512"
      version="1.1"
      viewBox="0 0 512 512"
      xmlSpace="preserve"
    >
      <g fill="#F2F2F2">
        <path d="M484.973 122.808L452.288 451.017 59.712 451.017 33.379 129.16 256 253.802z"></path>
        <path d="M473.886 60.983L256 265.659 38.114 60.983 256 60.983z"></path>
      </g>
      <path
        fill="#F14336"
        d="M59.712 155.493v295.524H24.139C10.812 451.017 0 440.206 0 426.878V111.967l39 1.063 20.712 42.463z"
      ></path>
      <path
        fill="#D32E2A"
        d="M512 111.967v314.912c0 13.327-10.812 24.139-24.152 24.139h-35.56V155.493l19.692-46.525 40.02 2.999z"
      ></path>
      <path
        fill="#F14336"
        d="M512 85.122v26.845l-59.712 43.526L256 298.561 59.712 155.493 0 111.967V85.122c0-13.327 10.812-24.139 24.139-24.139h13.975L256 219.792 473.886 60.983h13.962c13.34 0 24.152 10.811 24.152 24.139z"
      ></path>
      <path fill="#D32E2A" d="M59.712 155.493L0 146.235 0 111.967z"></path>
    </svg>
  );
}

export default IconGmail;